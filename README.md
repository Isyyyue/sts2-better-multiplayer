...
- 商店可反复交换金币，每笔由双方同时确认。
- 多人菜单可一键提交本 Mod 的固定结构诊断反馈，便于定位玩家端无法复现的问题。
- 双方确认后由房主程序自动校验并结算，不需要房主手动批准；消息带修订号与事务号，防止延迟和重复包复制物品。
- 房主掉线时沿用官方行为结束房间；本版不实现房主迁移。

## 环境

- Slay the Spire 2（Steam 正式版 `v0.107.1` 与 `public-beta v0.111.0`；发布前必须分别验证）
- .NET SDK 9
...
“发送反馈”只在玩家主动点击时提交一次固定结构的诊断事件，不会后台自动上传。事件可包含平台账号 ID、大厅 ID、房主/客户端角色、参与者及其角色 ID 和连接状态，以及金币交易、篝火交易与协助锻牌的参与双方、阶段和消息时序；同时包含游戏、本 Mod、BaseLib、运行时和窗口信息，以及报告者本机加载的 Mod ID 与版本。其他玩家电脑上的 Mod 环境不会由一份反馈推断或上传。

反馈不包含玩家昵称、房间名称、密码、IP 字段、本机路径、截图、存档、原始日志或玩家输入文本；不会写入磁盘等待以后重传。Sentry 的网络基础设施仍会像其他互联网服务一样在建立连接时看到来源 IP，但事件要求服务端不要推断或保存 IP。只有收到 Sentry 的 `2xx` 响应后，游戏内才显示“反馈已提交”和报告编号。
